...
const { PARAMS_ERROR, CAN_NOT_FOLLOW_ABOUT_YOURSELF } = require('../../util/error-type')
...
class UserMiddleware {
  ...
  async handleFollow(ctx, next) {
    ...
    if(!followId) return ctx.app.emit('error', new Error(PARAMS_ERROR), ctx)
    if(id == followId) return ctx.app.emit('error', new Error(CAN_NOT_FOLLOW_ABOUT_YOURSELF), ctx)
